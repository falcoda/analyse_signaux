...
def calc_img_approx_contour(img: Image):
    ...
    return removeClosedPoints(approx_raw, offset)
...
def get_ref_imgs(path):
    ...
    refs_path = os.listdir(path)
    if ('.DS_Store' in refs_path): 
      refs_path.remove('.DS_Store')
    
    for ref in refs_path:
        refs_img = os.listdir(path+"/"+ref)
        for ref_img in refs_img :
            newRefImg = Image()
            basic_manip(path + '/' +ref+ '/' + ref_img, newRefImg)
            newRefImg.name = ref_img
            
            img.contour = calc_img_contour(newRefImg)
            img.contour_approx =  calc_img_approx_contour(newRefImg)
...
if __name__ == '__main__':
    ...
    test_img_path= 'test/unknown_4.jpg'
    test_img = Image()
