...
def calc_textures(img: Image):
  textures = mt.features.haralick(img.grayscale)
  ht_mean = textures.mean(axis=0)
  img.contrast = ht_mean[1]
  img.correlation = ht_mean[2]
  img.entropy = ht_mean[8]


def compare_contrast(test_img:Image):
  global ref_images
  ...
  for ref_img in ref_images:
      ...
      diff = abs(ref_img.contrast - test_img.contrast)
      if diff < match_value:
          match_value = diff
          match_img = ref_img
...
def compare_correlation(test_img:Image):
  global ref_images
  ...
  for ref_img in ref_images:
      ...
      diff = abs(ref_img.correlation - test_img.correlation)
      if diff < match_value:
          match_value = diff
          match_img = ref_img
...
def compare_entropy(test_img:Image):
  global ref_images
  ...
  for ref_img in ref_images:
      ...
      diff = abs(ref_img.entropy - test_img.entropy)
      if diff < match_value:
          match_value = diff
          match_img = ref_img
...
def compare_area(test_img:Image):
  global ref_images
  ...
  for ref_img in ref_images:
      ...
      diff = abs(ref_img.area - test_img.area)
      if diff < match_value:
          match_value = diff
          match_img = ref_img
...
def compare_perimeter(test_img:Image):
  global ref_images
  ...
  for ref_img in ref_images:
      ...
      diff = abs(ref_img.perimeter - test_img.perimeter)
      if diff < match_value:
          match_value = diff
          match_img = ref_img
...
if __name__ == '__main__':
    ...
    test_img_path= 'test/unknown_3.jpg'
    test_img = Image()
    ...
    #match_list.append(compare_contrast(test_img))
    match_list.append(compare_correlation(test_img))
    match_list.append(compare_entropy(test_img))  
    #match_list.append(compare_area(test_img))  
    #match_list.append(compare_perimeter(test_img))  
